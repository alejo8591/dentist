...
<?php
   ...
   <br />
    ...
    <div id="intcpSilnessLoes">
        <?php
          foreach($model->intcpSilnessLoes as $id => $intcpSilnessLoes):
              $this->renderPartial('_intcpsilnessloe', array(
                  'model' => $intcpSilnessLoes,
                  'index' => $id,
                  'display' => 'block',
              ));
          endforeach;
   ...
   <div>
      <?php echo CHtml::link('Agregar - Indice de Placa Bacteriana Visible (Silness & Loes)', '#', array('id' => 'loadPISL')); ?>
   </div>
   <br />
...
<?php
   $index = 0;
      ...
      var _index = ' . $index . ';
      $("#loadPISL").click(function(e){
         e.preventDefault();
         var _url = "' . Yii::app()->controller->createUrl("loadPISL", array("load" => $this->action->id)) . '&index="+_index;
         $.ajax({
            url: _url,
            success:function(response){
               $("#intcpSilnessLoes").append(response);
               $("#intcpSilnessLoes .crow").last().animate({
                  opacity : 1, 
                   left: "+50", 
                   height: "toggle"
               });
